...
// Run:    ./build/silent_woods       (./build/silent_woods.exe on Windows)
//
// Controls:
//   WASD       — walk
//   Mouse      — look around
...
// On Windows, miniaudio includes <windows.h>, which defines many macros that
// collide with raylib's API (CloseWindow, Rectangle, DrawText, ShowCursor, etc).
// Strip them all here so the raylib names below resolve to raylib's functions
// and types, not the Win32 macros.
#ifdef _WIN32
    #undef CloseWindow
    #undef ShowCursor
    #undef LoadImage
...
static void DrawStalker(Vector3 p, float fearProximity) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (state == State::Playing) {
            ...
            if (eventTimer <= 0.0f) {
                ...
                eventTimer = 6.0f + (std::rand() % 100) / 10.0f;
            }
            windTimer -= dt;
            if (windTimer <= 0.0f) {
                PlaySound(sndWind);
        ...
        if (jumpscare > 0.0f) {
            unsigned char a = (unsigned char)std::fmin(255.0f, jumpscare * 220.0f);
        ...
        if (state == State::Menu) {
            ...
            DrawText("if it hears you, it comes closer", SCR_W / 2 + 20, 416, 22, {200,120,120,220});
